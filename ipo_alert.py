...
# C001=증권신고(지분증권), C006=소액공모(지분증권)
# 정정은 공시명에 "정정" 포함 여부로 구분
...
def get_dart_ipo(days=3):
    ...
    try:
        ...
        data = r.json()
        print("DART C001:", r.status_code, "총:", data.get("total_count", 0), "건")
        items = data.get("list", [])
        filtered = [i for i in items if not any(x in i.get("report_nm", "") for x in IPO_EXCLUDE)]
        print("필터 후:", len(filtered), "건")
        for i in filtered:
...
def get_full_details(rcept_no):
    ...
    try:
        ...
        target = next(
            (doc for doc in docs if "증권신고서" in doc.get("dc_nm", "") and "정정" not in doc.get("dc_nm", "")),
...
def main():
    ...
    # 1. DART 신규 증권신고서 (C001)
    for item in get_dart_ipo(days=3):
        nm = item.get("report_nm", "")
        is_amendment = "정정" in nm
        if item["rcept_no"] not in seen:
            send_telegram(fmt_dart_msg(item, is_amendment))
            seen.add(item["rcept_no"])
            print("DART:", item["corp_name"], "정정" if is_amendment else "신규")
            has_new = True
