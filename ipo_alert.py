...
def calc_rate(num, denom):
    try:
        n = float(re.sub(r'[^0-9]', '', str(num)))
        d = float(re.sub(r'[^0-9]', '', str(denom)))
        if d == 0:
            return ''
        return f"{n/d*100:.1f}%"
    except Exception:
...
def get_full_details(rcept_no):
    ...
    try:
        ...
        m = re.search(r'최대\s*주주[^가-힣]*([가-힣A-Za-z\s()]+?)\s+([0-9,\.]+)\s*%', plain)
        if m:
        ...
        for pat in [r'일반\s*청약[^0-9]*([0-9]{4}[.\-][0-9]{1,2}[.\-][0-9]{1,2})[^0-9~～\-]*[~～\-]\s*([0-9]{4}[.\-][0-9]{1,2}[.\-][0-9]{1,2})',
...
def fmt_dart_msg(item, is_amendment=False):
    try:
        ...
        if d["rev_3y"]:
            ...
            lines.append("<code>      | " + " | ".join(y[:4] for y in years[:len(d["rev_3y"])]))
...
def get_kind_items(days=7, count=20):
    try:
...
def fmt_summary(now_str, has_new, recent_kind, recent_dart):
    try:
        ...
        lines = ["✅ <b>IPO Alert 실행 완료</b> (" + now_str + ")", status, ""]
        ...
        if recent_kind:
            for i, item in enumerate(recent_kind, 1):
                ...
                lines.append(str(i) + ". " + emoji + " " + item["name"] + " | " + item["result"] + " | " + date_short)
        else:
            lines.append("  (KIND 조회 실패)")
        lines.append("")
        ...
        print("요약 메시지 오류:", e)
        return "✅ IPO Alert 실행 완료 (" + now_str + ")"
...
def main():
    ...
    # 3. KIND 신규 (7일)
    kind_all = get_kind_items(days=7, count=20)
    for item in kind_all:
        if item["id"] not in seen:
            send_telegram(fmt_kind_alert(item))
            seen.add(item["id"])
            print("KIND 신규:", item["name"], item["result"])
            has_new = True

    # 4. 확인 메시지 (KIND 최근 30일 3개 + DART 최근 3개)
    recent_kind = [i for i in get_kind_items(days=30, count=30) if "스팩" not in i["name"]][:3]
